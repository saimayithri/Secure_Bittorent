...
def start_peer(tracker_host, tracker_port, use_tls_requested):
    """Starts the Peer instance, passing the TLS request."""
    ...
    global log_handler
    # Ensure logger has the handler (it might have been removed on stop)
    if log_handler and log_handler not in streamlit_logger.handlers:
        streamlit_logger.addHandler(log_handler)

    streamlit_logger.info(f"Attempting to start Peer... TLS Requested: {use_tls_requested}")
    update_logs()

    try:
        # Pass use_tls_requested to the Peer constructor
        peer = Peer(tracker_host=tracker_host, tracker_port=tracker_port, use_tls=use_tls_requested)
        ...
        streamlit_logger.info(f"Peer process initialized. ID: {peer.peer_id[:8]}")
        st.success(f"Peer initialized: {peer.peer_id[:8]} on {peer.ip}:{peer.port}")
        update_logs()
        time.sleep(0.1)
        st.rerun()

    except Exception as e:
        st.error(f"Failed to start Peer: {e}", icon="🔥")
        streamlit_logger.critical(f"Failed to start Peer: {e}", exc_info=True)
        st.session_state.peer_instance = None
        st.session_state.peer_running = False
        update_logs()
...
def stop_peer():
    ...
    try:
        if peer_to_stop and hasattr(peer_to_stop, 'peer_id'):
            ...
            get_cached_peer_ui_data.clear()
            streamlit_logger.warning("Cleared UI data cache without specific peer ID.")
    except NameError: # Handle case where cache function might not be defined yet
        streamlit_logger.warning("Cache function 'get_cached_peer_ui_data' not found for clearing.")
    ...
    try:
        ...
        streamlit_logger.error(f"Error detaching log handler from Peer: {log_e}")

    # --- Call Peer Shutdown (with its own exception handling) ---
    try:
        if hasattr(peer_to_stop, 'shutdown') and callable(peer_to_stop.shutdown):
            peer_to_stop.shutdown()
            streamlit_logger.info("Peer shutdown method called.")
        else:
            streamlit_logger.error("Peer object missing 'shutdown' method.")
            st.error("Peer object missing 'shutdown' method.")
    except Exception as shutdown_err: # <-- *** This except block was missing ***
        st.error(f"Error during peer shutdown call: {shutdown_err}")
        streamlit_logger.error(f"Error during peer shutdown call: {shutdown_err}", exc_info=True)

    # --- Clear State (This should always run) ---
    finally:
        st.session_state.peer_instance = None
        st.session_state.peer_running = False
        streamlit_logger.info("Peer stopped and session state cleared.")
        # Clear caches (redundant if cache clearing worked above, but safe)
...
with st.sidebar:
    ...
    if st.session_state.peer_running and st.session_state.peer_instance:
        ...
        # Display the "Connected as" message (uses peer_id directly, which is fine)
        st.success(f"Connected as {peer.peer_id[:8]}")
        ...
        st.session_state.tls_toggle = st.checkbox(
            ...
            value=st.session_state.tls_toggle, # Control its state via session_state
            help="Requires vpn.py and cryptography library. Encrypts peer-to-peer traffic."
        )
        enable_tls = st.session_state.tls_toggle # Use the session state value
        # --- END ADDED TLS Toggle ---

        # Connect Button
        if st.button("Connect", key="connect_button"):
            if tracker_host and tracker_port > 0:
                if not st.session_state.get('peer_running', False):
                    # Pass the current state of the toggle to start_peer
                    start_peer(tracker_host, tracker_port, enable_tls)
                else:
...
if st.session_state.peer_running and st.session_state.peer_instance:
    peer = st.session_state.peer_instance
        # --- MODIFIED: Call the cached function ONCE here ---
    # Use the peer's actual peer_id as the argument for the cache function
    cached_data = get_cached_peer_ui_data(peer.peer_id)
    # Extract data for easier use in tabs and sidebar (if needed here)
    ...
    # Use Peer's SCRIPT_DIR if available, otherwise fallback
    ...
    try:
        ...
        shared_dir_rel = os.path.relpath(shared_dir_path, start=start_path)
        downloads_dir_rel = os.path.relpath(downloads_dir_path, start=start_path)
    except ValueError:
        # Fallback to absolute paths if they are on different drives (Windows)
        shared_dir_rel = shared_dir_path
        downloads_dir_rel = downloads_dir_path
                    ...
                    if selected_file_info:
                        ...
                        if not downloads_dir_exists:
                            ...
                            if st.button("Attempt Create Downloads Dir", key=f"create_dl_dir_{selected_hash}"):
                                try: os.makedirs(downloads_dir_path, exist_ok=True); st.success(f"Created downloads directory: {downloads_dir_path}"); st.rerun()
                                except OSError as e: st.error(f"Could not create downloads dir: {e}"); streamlit_logger.error(f"Could not create {downloads_dir_path}: {e}")

                        # Only show download options if directory exists
                        if downloads_dir_exists:
                            ...
                            with col_dl_params1:
                                # Use default values from peer module if available
                                default_delay = DEFAULT_CHUNK_DOWNLOAD_DELAY_SECONDS if 'DEFAULT_CHUNK_DOWNLOAD_DELAY_SECONDS' in globals() else 0.0
                                dl_delay = st.number_input("Debug Delay per chunk (s)", 0.0, value=default_delay, step=0.1, format="%.1f", help="Artificial delay between chunk downloads by a worker.", key=f"delay_{selected_hash}")
                            with col_dl_params2:
                                default_parallel = DEFAULT_MAX_PARALLEL_DOWNLOADS if 'DEFAULT_MAX_PARALLEL_DOWNLOADS' in globals() else 4
                                max_parallel = st.number_input("Max parallel chunks", 1, value=default_parallel, step=1, help="Max concurrent chunk download workers for this file.", key=f"parallel_{selected_hash}")

                            # Sanitize filename and check for existing file/temp
                            proceed_with_download, allow_proceed_if_exists = False, False
                            sanitized_name = default_filename # Start with default
                            if save_name: # If user entered something
                                # Basic sanitization
                                temp_sanitized = save_name.replace('/', '_').replace('\\', '_')
                                # Allow common characters
                                valid_chars = "-_.() abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
                                temp_sanitized = "".join(c for c in temp_sanitized if c in valid_chars).strip()
                                # Use sanitized name or fallback if empty/invalid
                            ...
                            if file_exists or temp_exists:
                                st.warning(f"File '{sanitized_name}' or `.tmp` already exists in downloads folder.")
                                confirm_overwrite = st.checkbox("Confirm Overwrite/Resume?", key=f"overwrite_cb_{selected_hash}", value=False, help="If checked, will overwrite final file or resume from .tmp file.")
                                if confirm_overwrite:
                                    allow_proceed_if_exists = True
                                else:
                                    st.caption("Check box and click 'Start Download' again to overwrite or resume.")
                            else:
                                proceed_with_download = True # File doesn't exist, safe to proceed

                            # Start Download Button Logic
                            if st.button("Start Download", key=f"start_dl_{selected_hash}"):
                                if not save_name: st.warning("Please enter a filename.")
                                elif file_exists or temp_exists: # Re-check existence case
                                    if allow_proceed_if_exists:
                                        streamlit_logger.info(f"Overwrite/Resume confirmed for {sanitized_name}.")
                                        # No need to manually delete, peer.download_file handles resume/overwrite based on .tmp size
                                        proceed_with_download = True
                                    else: st.warning("Overwrite/Resume not confirmed."); proceed_with_download = False
                                else: proceed_with_download = True # File didn't exist initially

                                # Initiate download if okay to proceed
                                if proceed_with_download:
                                    streamlit_logger.info(f"Proceeding with download initiation: '{sanitized_name}'")
                                    try:
                                        if not hasattr(peer, 'download_file'): st.error("Peer missing 'download_file'"); streamlit_logger.error("Missing 'download_file'")
                                            ...
                                            with st.spinner(f"Initiating download for '{sanitized_name}'..."):
                                                # Pass absolute save path and parameters
                                                success = peer.download_file(
                                                    file_hash=selected_hash,
                                                    save_path=os.path.abspath(save_path), # Pass absolute path
                                                    download_delay=dl_delay,
                                                    max_parallel=max_parallel
                                                )
                                            if success: st.success(f"Download initiated/resumed for '{sanitized_name}'. Check Progress tab."); streamlit_logger.info(f"Download initiated/resumed: {sanitized_name}")
    ...
    # --- Progress Tab ---
    # --- Progress Tab (MODIFIED: Use cached data, Add Cancel Button with Error Handling, Improve message) ---
    with tab_progress:
        st.subheader("📊 Live Download Progress")
        # --- Use cached download status ---
        active_downloads = active_downloads_cached # Fetched earlier
        if not active_downloads:
            # --- IMPROVED MESSAGE ---
            st.info("No active downloads. Check the 'Downloads' tab for completed files.")
        else:
            sorted_downloads = sorted(active_downloads.items(), key=lambda item: item[1].get('filename', ''))
            for file_hash, info in sorted_downloads: # Loop through downloads
                if not isinstance(info, dict): continue
                # --- Outer TRY for displaying this download's info ---
                try:
                    # Extract info (filename, state, etc.)
                    ...
                    # --- Use 4 columns for Metrics and Button ---
                    cols=st.columns(4)
                    ...
                    with cols[3]:
                        # Show cancel button only if download is in a cancellable state
                        # Allow cancelling FAILED downloads too, to remove the entry
                        if state in ["DOWNLOADING", "FINDING_PEERS", "PROCESSING", "UNKNOWN", "FAILED"]:
                            st.markdown('<div style="margin-top: 28px;"></div>', unsafe_allow_html=True) # Vertical alignment
                            if st.button("Cancel", key=f"cancel_dl_{file_hash}", type="secondary", use_container_width=True, help="Stop this download and delete .tmp file."):
                                streamlit_logger.info(f"User cancel request: {file_hash[:8]}")
                                cancelled = False # Default cancel state
                                # Check if cancel_download method exists before calling
                                if hasattr(peer, 'cancel_download') and callable(peer.cancel_download):
                                    # --- Wrap the actual cancellation call in try/except ---
                                    try:
                                        with st.spinner(f"Cancelling '{filename}'..."):
                                            cancelled = peer.cancel_download(file_hash) # Call the peer method
                                    ...
                                    # --- End try/except for cancellation call ---

                                    # Handle result after spinner finishes
                                    if cancelled:
                                        st.success(f"Cancelled '{filename}'.")
                                        get_cached_peer_ui_data.clear() # Clear cache
                                        time.sleep(0.5); st.rerun()
                                    else:
                                        # Only show warning if cancel was attempted but peer returned false
                                        if not cancel_err: # Check if an exception occurred above
                                             st.warning(f"Could not cancel '{filename}' (Peer returned false).")
                                else: # hasattr check failed
                                    st.error("Peer object needs 'cancel_download' method.")
                                    streamlit_logger.error("Peer object missing 'cancel_download' method.")
                        else: # state is not one where cancel makes sense (e.g., COMPLETED)
                            st.write("") # Empty placeholder to maintain column structure/alignment

                    # --- Chunk Details Expander ---
                    ...
                    st.divider() # Divider after displaying this download entry

                # --- EXCEPT corresponding to the outer TRY (for displaying the whole entry) ---
                except Exception as e:
                    # Display error for this specific download entry without crashing the whole loop
                    st.error(f"Error displaying progress for {filename}: {e}")
                    streamlit_logger.error(f"Error display progress {file_hash[:8]}: {e}", exc_info=True) # Log with traceback
    ...
    # --- Auto Refresh Logic ---
    # Only rerun if the peer is supposed to be running to prevent loops when disconnected
    if st.session_state.peer_running:
