...
# --- Peer Class ---
class Peer:
    def __init__(self, tracker_host='localhost', tracker_port=5000, use_tls: bool = False):
        """Initializes the Peer instance."""
        self.peer_id = hashlib.sha1(str(random.random()).encode()).hexdigest()
        self.running = True
        self.logger = self.setup_logging() # Logger setup FIRST
        self.lock = threading.Lock() # Main lock for shared state
        ...
        self.setup_directories() # Create directories if they don't exist

        # Network Info
        self.ip = self.get_local_ip() # Determine local IP
        self.port = None # Port will be determined by _find_available_port
        for attempt in range(3): # Try finding port multiple times
        ...
        # TLS Initialization
        self.use_tls = False # Default to False (will be updated if setup succeeds)
        self.server_ssl_context = None
        self.client_ssl_context = None

        # Only attempt TLS setup if requested *and* dependencies are met
        if use_tls and VPN_AVAILABLE and CRYPTOGRAPHY_AVAILABLE:
            self.logger.info("Attempting to initialize TLS as requested...")
            try:
                if vpn.ensure_self_signed_cert(self.peer_id):
                    self.server_ssl_context = vpn.create_server_context()
                    self.client_ssl_context = vpn.create_client_context()
                    if self.server_ssl_context and self.client_ssl_context:
                        # Set self.use_tls to True *only if* setup was successful
                        self.use_tls = True
                        self.logger.info("TLS contexts created successfully and enabled.")
                    else:
            ...
            # Log why TLS was requested but not enabled
        ...
        self.initialize_shared_files() # Scan shared dir and publish initial files
    ...
    def _get_peers_for_chunks(self, chunk_hashes: List[str]) -> Dict[str, List[Dict]]:
        """Requests peer lists for multiple chunk hashes from the tracker."""
        if not chunk_hashes: return defaultdict(list)
        # Remove duplicates before querying tracker
        ...
        # Query tracker for each unique hash
        # TODO: Potential optimization: Batch requests to tracker if supported
        for chash in unique_hashes:
             if not self.running: break # Stop if peer is shutting down
             try:
                 response = self.send_to_tracker_with_retry({'type': 'get_peers', 'chunk_hash': chash, 'peer_id': self.peer_id})
                 if response.get('status') == 'success':
                     # Filter peers: must be dict, have required keys, not be self
                     valid_peers = [
                         { # Reconstruct dict to ensure all keys present and default tls_capable
                     ...
                     # else: self.logger.debug(f"No valid peers returned for chunk {chash[:8]}")
                 # else: self.logger.warning(f"Tracker returned error for get_peers {chash[:8]}: {response.get('message')}")
             except Exception as e:
    ...
    def _download_chunk_from_peer(self, peer_info: Dict, chunk_info: Dict, file_hash: str) -> bool:
        ...
        try:
            ...
            # Attempt TLS upgrade if enabled and peer supports it
            if self.use_tls and self.client_ssl_context and peer_info.get('tls_capable', False):
                self.logger.debug(f"[{thread_name}] Attempting TLS handshake with {peer_id[:8]}...")
                try:
                    sock = self.client_ssl_context.wrap_socket(plain_sock, server_side=False, server_hostname=peer_ip) # Use IP for SNI/hostname check if needed
                    sock.settimeout(BASE_TIMEOUT) # Handshake timeout
                    sock.do_handshake()
                    self.logger.debug(f"[{thread_name}] TLS handshake successful with {peer_id[:8]}.")
                    ...
                    try: plain_sock.close() # Ensure original socket is closed on failure
                    except Exception: pass
                    return False # Treat TLS failure as download attempt failure
            # If TLS not used or failed, sock remains the plain_sock (or is None if connect failed)

            # Set timeout for subsequent operations on the established socket
            ...
            self.logger.debug(f"[{thread_name}] Sending request for chunk {chunk_hash[:6]} to {peer_id[:8]}.")
            ...
            if len(header) != 45:
                self.logger.warning(f"[{thread_name}] Failed to receive full header from {peer_id[:8]} (got {len(header)} bytes). Peer may be busy/choked or disconnected.")
                return False
            ...
            if resp_type != b'D': # 'D' = Data response type expected
                self.logger.debug(f"[{thread_name}] Peer {peer_id[:8]} denied chunk {chunk_hash[:6]} (Type: {resp_type!r}). Possibly choked.")
            ...
            if chunk_data_size > 0:
                ...
                self.logger.debug(f"[{thread_name}] Receiving {chunk_data_size} bytes from {peer_id[:8]} (Timeout: {data_receive_timeout}s)...")
            ...
            if self.save_chunk(file_hash, chunk_info, chunk_data):
                ...
                self.logger.debug(f"[{thread_name}] Downloaded chunk {chunk_index} ({dl_bytes} bytes in {duration:.2f}s) from {peer_id[:8]}.")
            ...
            self.logger.debug(f"[{thread_name}] Socket timeout during communication with {peer_id[:8]} for chunk {chunk_index}.")
            return False
        except (ssl.SSLError, ConnectionError, BrokenPipeError, OSError) as e:
             # Consolidate common network errors
             self.logger.debug(f"[{thread_name}] Network error with {peer_id[:8]} for chunk {chunk_index}: {type(e).__name__}")
             return False
        except Exception as e:
            # Catch any other unexpected errors during the process
            self.logger.error(f"[{thread_name}] Unexpected error downloading chunk {chunk_index} from {peer_id[:8]}: {e}", exc_info=False) # Less verbose traceback for workers
            return False
        finally:
    ...
    def accept_connections(self):
        """Main loop for listening and accepting incoming peer connections."""
        thread_name = threading.current_thread().name
        tls_status = 'ENABLED' if self.use_tls else 'DISABLED'
        self.logger.info(f"[{thread_name}] Acceptor thread started (TLS: {tls_status}). Listening...")

        while self.running:
            server_sock = self.server_socket # Get current server socket reference
            if not server_sock:
                # Handle case where socket might have been closed due to error
                if not self.running: break # Exit if shutting down
                self.logger.warning(f"[{thread_name}] Server socket is closed. Attempting restart in 5s...")
                time.sleep(5)
                self.server_socket = self.setup_server_socket() # Try to re-establish
            ...
            try:
                ...
                try:
                    ...
                    continue # Normal timeout, loop again to check self.running

                peer_ip, peer_port = addr
                self.logger.debug(f"[{thread_name}] Accepted connection from {peer_ip}:{peer_port}")
                conn.settimeout(BASE_TIMEOUT) # Set timeout for initial operations

                # --- TLS Handling ---
                handler_socket = conn # Assume plain TCP first
                if self.use_tls and self.server_ssl_context:
                     self.logger.debug(f"[{thread_name}] Attempting TLS wrap for {peer_ip}:{peer_port}")
                     try:
                         # Wrap the socket for TLS server-side
                         handler_socket = self.server_ssl_context.wrap_socket(conn, server_side=True)
                         handler_socket.settimeout(BASE_TIMEOUT) # Timeout for TLS handshake
                         # Handshake happens implicitly on first read/write, or explicitly:
                         # handler_socket.do_handshake()
                         self.logger.debug(f"[{thread_name}] TLS wrap successful for {peer_ip}:{peer_port}")
                     except (ssl.SSLError, socket.timeout, ConnectionError, OSError, Exception) as ssl_err:
                          self.logger.warning(f"[{thread_name}] TLS wrap/handshake failed for {peer_ip}:{peer_port}: {ssl_err}")
                          try: conn.close() # Close original socket if wrap failed
                          except Exception: pass
                ...
                handler_thread = threading.Thread(target=self.handle_peer_connection,
                                                  args=(handler_socket, addr),
                                                  name=handler_name, daemon=True)
                handler_thread.start()

            except OSError as e:
                # Handle potential socket errors during accept (e.g., socket closed)
                if not self.running: break # Expected if shutting down
                ...
                time.sleep(2) # Wait before attempting re-setup
            except Exception as e:
    ...
    def process_peer_json_message(self, conn: socket.socket, addr: Tuple[str, int], message: Dict, requesting_peer_id: str):
        ...
        if msg_type == 'chunk_request':
            self.handle_chunk_request(conn, addr, message, requesting_peer_id)
        # --- Add handlers for other P2P messages here ---
        # Example:
        # elif msg_type == 'have':
        #     self.handle_have_message(addr, message, requesting_peer_id)
        # elif msg_type == 'interested':
        #     self.handle_interested_message(addr, message, requesting_peer_id)
        # elif msg_type == 'not_interested':
        #     self.handle_not_interested_message(addr, message, requesting_peer_id)
        # elif msg_type == 'choke':
        #     self.handle_choke_message(addr, message, requesting_peer_id)
        # elif msg_type == 'unchoke':
        #      self.handle_unchoke_message(addr, message, requesting_peer_id)
        # --------------------------------------------------
        else:
            self.logger.warning(f"[{thread_name}] Received unknown JSON message type '{msg_type}' from {requesting_peer_id[:8]}.")

    def handle_chunk_request(self, conn: socket.socket, addr: Tuple[str, int], message: Dict, requesting_peer_id: str):
        """Handles a 'chunk_request' message from a peer, sending data if available and unchoked."""
        thread_name = threading.current_thread().name
        chunk_hash = message.get('chunk_hash')

        # Validate request
        if not isinstance(chunk_hash, str) or len(chunk_hash) != 40 or requesting_peer_id == "Unknown":
            self.logger.warning(f"[{thread_name}] Invalid chunk_request received: {message}")
            return

        self.logger.debug(f"[{thread_name}] Received chunk request for {chunk_hash[:8]} from {requesting_peer_id[:8]}.")

        filepath = None; chunk_metadata = None; is_unchoked = False; read_from_temp = False

        # --- Check Availability and Choking Status (Under Lock) ---
        with self.lock:
            # 1. Check if requester is unchoked by us
            is_unchoked = requesting_peer_id in self.unchoked_upload_peers
            if not is_unchoked:
                # Don't send data if choked, just log and return
                self.logger.debug(f"[{thread_name}] CHOKED request for {chunk_hash[:6]} from {requesting_peer_id[:8]}. Ignoring.")
                # Optionally send a CHOKE message back? Standard BT doesn't require response to request when choked.
                return

            # 2. Check if we have the requested chunk and find its location
            if chunk_hash in self.available_chunks:
                # Prioritize fully shared files first
                file_hash_source = self.chunk_sources.get(chunk_hash) # Get set of file_hashes providing this chunk
                if file_hash_source:
                    found_in_shared = False
                    # Check if any source is a fully shared file path
                    for f_hash_src in file_hash_source:
                        # Need to map file_hash back to path
                        for fpath, finfo in self.shared_files.items():
                            if finfo.get('hash') == f_hash_src:
                                # Found in shared files, get specific chunk meta
                    ...
                    # If not found in shared, check active downloads (temp files)
                             ...
                             if f_hash_src in self.downloads:
                                 ...
                                 for chunk_meta in dl_info.get('chunk_objects', []):
                                     if chunk_meta.get('hash') == chunk_hash and chunk_meta.get('status') == 'complete':
                                         ...
                                         self.logger.debug(f"[{thread_name}] Serving chunk {chunk_hash[:6]} from temp file of DL {f_hash_src[:6]}")
                                         break
                                 if filepath: break # Found in a download

            # 3. Validate found data location and metadata
            if not filepath or not chunk_metadata:
                self.logger.warning(f"[{thread_name}] Chunk {chunk_hash[:6]} requested by {requesting_peer_id[:8]} is marked available but not found in shared/downloads state!")
                # Possible state inconsistency, maybe send error or ignore? Ignoring for now.
                return
            offset = chunk_metadata.get('offset', -1)
            size = chunk_metadata.get('size', -1)
            if offset < 0 or size < 0:
                self.logger.error(f"[{thread_name}] Invalid metadata found for chunk {chunk_hash[:6]}. Offset:{offset}, Size:{size}")
                return

        # --- Read and Send Data (Outside Lock) ---
        try:
            # Re-check file existence just before reading
            if not os.path.exists(filepath):
                self.logger.warning(f"[{thread_name}] File disappeared before sending chunk: '{os.path.basename(filepath)}'. Chunk:{chunk_hash[:6]}")
                # TODO: Should potentially update internal state here (e.g., trigger verify_shared_files)
                return

            # Read chunk data from file
            self.logger.debug(f"[{thread_name}] Reading chunk {chunk_metadata.get('index','?')} ({size} bytes) from '{os.path.basename(filepath)}'...")
            with open(filepath, 'rb') as f:
                lock_file(f) # Best-effort file lock
                try:
                    f.seek(offset)
            ...
            if len(chunk_data) != size:
                self.logger.error(f"[{thread_name}] Read incorrect size for chunk {chunk_hash[:6]} from '{os.path.basename(filepath)}'. Got {len(chunk_data)}, expected {size}")
                return

            # Construct header: Type(1)=D + Hash(40) + Size(4)
            header = b'D' + chunk_hash.encode('ascii') + size.to_bytes(4,'big')
            # Send header and data
            self.logger.debug(f"[{thread_name}] Sending chunk {chunk_metadata.get('index','?')} header and data ({size} bytes) to {requesting_peer_id[:8]}...")
            conn.sendall(header + chunk_data)

            # Update upload statistics under lock
            with self.lock:
                self.peer_upload_stats[requesting_peer_id]['bytes_up_interval'] += size

            source_type = 'temp' if read_from_temp else 'shared'
            self.logger.info(f"[{thread_name}] SENT chunk {chunk_metadata.get('index','?')} ({chunk_hash[:6]}, {size}b, {source_type}) -> {requesting_peer_id[:8]}.")

        except (IOError, OSError) as e:
             self.logger.warning(f"[{thread_name}] IO/OS Error sending chunk {chunk_hash[:8]} to {requesting_peer_id[:8]}: {e}")
        except (ConnectionError, BrokenPipeError, ssl.SSLError) as e:
             self.logger.debug(f"[{thread_name}] Network error sending chunk {chunk_hash[:8]} to {requesting_peer_id[:8]}: {type(e).__name__}")
        except Exception as e:
            self.logger.error(f"[{thread_name}] Unexpected error sending chunk {chunk_hash[:8]} to {requesting_peer_id[:8]}: {e}", exc_info=True)
    ...
    def maintenance_tasks(self):
        ...
        while self.running:
            ...
            try:
                # --- Frequent Tasks ---
                # Choking Recalculation (driven by its own interval check)
                self.recalculate_choking()

                # Download Speed Calculation (runs every MAINTENANCE_INTERVAL cycle)
                with self.lock:
                    for fhash, info in self.downloads.items():
                         time_since_last_calc = now - info.get('last_speed_calc_time', now)
                         # Calculate speed if interval passed or first time
                         if time_since_last_calc >= SPEED_CALC_INTERVAL:
                             ...
                             info['current_speed_bps'] = current_speed_bps # Update speed
                             info['bytes_since_last_calc'] = 0 # Reset counter
                             info['last_speed_calc_time'] = now # Update timestamp
                ...
                # Catch errors within the maintenance loop to prevent thread death
                self.logger.error(f"[{thread_name}] Error during maintenance cycle: {e}", exc_info=True)

            # Sleep until roughly the next maintenance interval
            elapsed = time.time() - start_time
            sleep_time = max(0.1, MAINTENANCE_INTERVAL - elapsed) # Ensure minimum sleep
